...
import airfoil.bezier as b
import airfoil.xfoil as xf
import os
import errno
...
def generate_airfoil(control_points_list,airfoil_file_name, path, method):
    if method=="bezier":
        ...
        xvals, yvals = b.bezier_curve(control_points_list, nTimes=100)
...
def xfoil(control_points_list, airfoil_file_name, params, method, timeout, graphics):
    ...
    command = xf.Command(airfoil_file_name, params)
    command.run(timeout, graphics)
...
if __name__ == "__main__":
    ...
    results_dir = r'./test_opt/Nelder-Mead/'
    make_sure_path_exists(results_dir)
    # params = 5, 0.2, 5e005, results_dir
    # command = xf.Command('0', params)
    # command.run(timeout=10, graphics=True)
    # ldc = command.getLDfromLog()

    #test xfoil()##############
    ys = [0.05, 0.1, 0.2, -0.2, -0.1, -0.05]
    control_points_list = prepare_controls(ys)
    params = 5, 0.2, 5e005, results_dir
    ldc = xfoil(control_points_list, '10', params, method= 'bezier', timeout=5, graphics=True)
